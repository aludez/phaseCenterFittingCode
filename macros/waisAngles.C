...
void waisAngles(int runStart, int runEnd)
{
	...
	TH2D* h = new TH2D("h", "h", 2, 0, 1.01, 360, -60, 60);
	...
	for(Long64_t entry=0; entry < maxEntry; entry++)
	{
		...
		double distToWais = usefulPat->getDistanceFromSource(AnitaLocations::LATITUDE_WAIS_A4, AnitaLocations::LONGITUDE_WAIS_A4, AnitaLocations::ALTITUDE_WAIS_A4);
		usefulPat->getThetaAndPhiWave(AnitaLocations::LATITUDE_WAIS_A4, AnitaLocations::LONGITUDE_WAIS_A4, AnitaLocations::ALTITUDE_WAIS_A4, thetaWave, phiWave);
		double timeToWais = 1e9 * distToWais/299792458;
		double ttnsDeltaH = ttnsDelta + 10e3;
		Int_t HV = -1;
		thetaWave *= 180/TMath::Pi();
		if((TMath::Abs(ttnsDelta) < cutTimeNs)) HV = 0; 
		if((TMath::Abs(ttnsDeltaH) < cutTimeNs)) HV = 1; 
		if(HV == -1) continue; 
//		printf("%d, %g\n", HV, thetaWave); 
	...
	TFile f(Form("theta_%dto%d.root", runStart, runEnd), "RECREATE");
	h->Write();
	h->Draw();
	//g->Draw("ap");
	delete evChain;
